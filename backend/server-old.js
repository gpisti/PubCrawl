...
app.post('/api/routes/:id/join', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Route not found or user already a participant' });
    }
...
app.post('/api/routes/:id/leave', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE id = $1 AND $2 = ANY(participants) AND owner_id != $2
      RETURNING *
    `, [id, userId]);
    
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Route not found, user not a participant, or user is owner' });
    }
